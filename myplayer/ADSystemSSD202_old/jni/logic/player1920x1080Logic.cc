...
void onNetChange1920x1080(char* netType){
...
//    MEDIAMANAGER->initPlayList();
	initNetManager(onNetChange1920x1080);
...
//	mTVVersionPtr->setText(strcat(APP_VERSION, "-sstar"));
	mTVVersionPtr->setText("V1.0.3-sstar");
	SNMANAGER->initUpdateParam(mQRCode1Ptr);
	MODELMANAGER->parseParamsTask(mQRCode1Ptr);
	MEDIAPLAYER->showPlayer(false);
//	MEDIAPLAYER->setPlayerVol(-50);
}
 ...
 *            触摸事件将继续传递到控件上
 */
static bool onplayer1920x1080ActivityTouchEvent(const MotionEvent &ev) {
    switch (ev.mActionStatus) {
		...
		case MotionEvent::E_ACTION_UP:  //触摸抬起
			...
			LOGD("up - 时刻 = %ld  %ld 坐标  x = %d, y = %d", time, downTime, ev.mX, ev.mY);
			if(time > 3000){
